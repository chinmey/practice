...
we flag that it is possible to partition array into K parts with equal sum, because remaining elements already have a sum equal to required sum.
...
{ 
	...
	subsetSum[0] = arr[N - 1]; /* optional step*/
	taken[N - 1] = true;  /* optional step*/
