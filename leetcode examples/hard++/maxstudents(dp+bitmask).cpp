...
my attempt(38/55 pass hua tha)
class Solution {
public:
    
    bool safe(int n,int m,int cur_r,int cur_c)
{
    return cur_r<n&&cur_r>=0&&cur_c>=0&&cur_c<m;
}
bool secure(vector<vector<char>>& seats,int n,int m,int cur_r,int cur_c)
{
    /*if(cur_r>=n||cur_r<0||cur_c<0||cur_c>=m) return true;
    if(arr[cur_r][cur_c]==1 && arr[cur_r][cur_c]==0)
    return true;
    if(arr[cur_r][cur_c]==2) return false;*/
    
      return !(cur_r<n&&cur_r>=0&&cur_c>=0&&cur_c<m&&(seats[cur_r][cur_c]=='@'));
    ...
    int maxStudents(vector<vector<char>>& seats) {
        ...
        for(i=0;i<n;i++)
        {
            for(j=0;j<m;j++)
            {
                if(seats[i][j]!='#')
                {
                    if(safe(n,m,i,j)&&secure(seats,n,m,i,j+1)&&secure(seats,n,m,i,j-1)&&secure(seats,n,m,i-1,j+1)&&secure(seats,n,m,i-1,j-1))
                    {
                        seats[i][j]='@';
                    }
                }
...
-------------------------------------------------------------------------------------------------------------------------------------
correct approach(https://leetcode.com/problems/maximum-students-taking-exam/discuss/503686/a-simple-tutorial-on-this-bitmasking-problem)
...
What is bitmasking? Bitmasking is something related to bit and mask. For the bit part, everything is encoded as a single bit, 
so the whole state can be encoded as a group of bits, 
i.e. a binary number. For the mask part, we use 0/1 to represent the state of something. 
In most cases, 1 stands for the valid state while 0 stands for the invalid state.

Let us consider an example. There are 4 cards on the table and I am going to choose several of them. We can encode the 4 cards as 4 bits.
 Say, if we choose cards 0, 1 and 3,
 we will use the binary number "1011" to represent the state. If we choose card 2, we will use the binary number "0100" then. 
 The bits on the right represent cards with smaller id.

As we all know, integers are stored as binary numbers in the memory but appear as decimal numbers when we are coding. As a result,
 we tend to use a decimal number instead of a binary number to represent a state. In the previous example, we would use "11" and "4"
  instead of "1011" and "0100".

When doing Bitmasking DP, we are always handling problems like
 "what is the i-th bit in the state" or "what is the number of valid bits in a state". 
These problems can be very complicated if we do not handle them properly. I will show some coding tricks below which we can make use of and solve this problem.
...
Now we can come to the problem. We can use a bitmask of n bits to represent the validity of each row in the classroom.
 The i-th bit is 1 if and only if the i-th seat is not broken. For the first example in this problem, the bitmasks will be "010010", "100001" and "010010".
  When we arrange the students to seat in this row, we can also use n bits to represent the students. The i-th bit is 1 if and only if the i-th seat is occupied by a student. 
